...
; -----
; Using substitution, we find that (add-1 zero) concludes to
; one = (lambda (f) (lambda (x) (f x)))
(define one (lambda (f) (lambda (x) (f x))))
; and doing (add-1 one) concludes to
; two = (lambda (f) (lambda (x) (f (f x))))
(define two (lambda (f) (lambda (x) (f (f x)))))

; from http://en.wikipedia.org/wiki/Church_encoding#Church_numerals
; we see that we need f^n1(f^n2(x)) to add n1 + n2
(define (add n1 n2)
